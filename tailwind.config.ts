...
function customBackgroundUtilities({ matchUtilities }) {
  const colorsObject = colors || {}; // Ensure colors object is defined
  matchUtilities(
    {
      "bg-grid": (value) => ({
    ...
    { values: Object.keys(colorsObject), type: "color" }
  );
}
...
function addVariablesForColors({ addBase }) {
  ...
  const newVars = Object.fromEntries(
    Object.entries(colorsObject).map(([key, val]) => [`--${key}`, val])
  );
